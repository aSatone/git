    ...
    <el-table :data="books" style="width: 100%; margin-top: 20px;">
      ...
      <el-table-column prop="price" label="价格/元" width="200">
        <template v-slot="scope">
          <el-input v-model="scope.row.price" @change="updateBook(scope.row.id, 'price', scope.row.price)"></el-input>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
const updateBook = async (id, field, value) => {
  try {
    const response = await axios.put(`http://localhost:9999/books/${id}`, {
      [field]: value
    })
